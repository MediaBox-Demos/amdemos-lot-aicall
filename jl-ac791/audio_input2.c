...
#define _AUDIO_TASK_NAME    "audio_task2"
...
static const int PLAY_BUFFER_DURATION = 40;
static const int PLAY_BUFFER_REMAIN_DURATION = 40;
//解码器读取PCM数据
static int audio_play_net_vfs_fread(VOID *file, VOID *data, unsigned int len)
{
    cbuffer_t *cbuf = (cbuffer_t *)file;
    unsigned int cbuf_len = cbuf_get_data_size(cbuf);

    if (cbuf_len == 0) {
        u32 begin = timer_get_ms();
        const unsigned int init_buffer_size = (PLAY_BUFFER_DURATION + PLAY_BUFFER_REMAIN_DURATION) * SAMPLE_RATE * CHANNEL * BIT_DEP / 8 / 1000;
        while (cbuf_len < init_buffer_size && g_audio_hdl.is_audio_play_open) {
            os_sem_pend(&g_audio_ctrl.r_sem, _AUDIO_WAIT_TIMEOUT);
            cbuf_len = cbuf_get_data_size(cbuf);
        }
        u32 end = timer_get_ms();
        audio_debug("audio no data in cbuf, wait %d ms for len %d, got len %d\n", end - begin, len, cbuf_len);
    }

    unsigned int rlen = cbuf_len > len ? len : cbuf_len;
    if (rlen == 0) {
        audio_debug("AUDIO NO DATA IN CBUF! for len %d\n", len);
        return 0;
    }
    //返回成功读取的字节数
    return cbuf_read(cbuf, data, rlen);
...
static VOID audio_player_net_init()
{
    ...
    req.dec.output_buf_len  = PLAY_BUFFER_DURATION * SAMPLE_RATE * CHANNEL * BIT_DEP / 8 / 1000;
    req.dec.priority        = 1;
...
//*****************************收音和播放************************
VOID _device_net_audio2(BOOL_T flag)
{
...
VOID _device_net_audio_play2(BOOL_T flag)
{
...
VOID _device_net_audio_recorder2(BOOL_T flag)
{
...
//***************************************************************
BOOL_T is_audio_play_open2(VOID)
{
...
int audio_cfg_init2(_AUDIO_PARAM *audio_param)
{
...
VOID _audio_init2(_AUDIO_PARAM *audio_param)
{
    audio_debug("into volc audio inti!");
    static char init = 0;

    audio_cfg_init2(audio_param);
    if (!init) {
        _audio_soft_init();
    }
...
void audio_stream_init2(int sample_rate, int bit_dept, int channel_num)
{
    ...
    _audio_init2(&audio_param);
}

void start_audio_stream2(void)
{
    _device_net_audio2(true);
}

void stop_audio_stream2(void)
{
    _device_net_audio2(false);
}
